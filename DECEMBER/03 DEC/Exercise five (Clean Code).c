...
void printNameVertical(char *name)
{
    ...
    { // O loop é executado até que o caractere atual seja '\0'
        printf("%c\n", name[i]);
    }
}
...
void printWordReversed(char *word)
{
    ...
    { // O loop é executado de trás para a frente
        printf("%c", word[i]);
    }
    printf("\n");
}
...
int countVowels(char *word)
{
    ...
    { // Vê todos os caracteres da palavra
        for (int j = 0; j < strlen(vowels); j++)
        { // Vê todas as vogais da variável vowels
            if (word[i] == vowels[j])
            {
                count++;
            }
...
void identifyOperator(char *phoneNumber)
{
    ...
    { // phoneNumber[0] verifica o primeiro caractere do número
    case '0':
